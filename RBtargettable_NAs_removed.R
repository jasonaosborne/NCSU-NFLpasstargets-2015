# this code is based on the targetdata dataset from Graham's handout on Thurs
...
n.years <- length(unique(targetdata$Year))
...
sorted.year <- rev(unique(targetdata$Year))
...
rownames(tgts) <- rev(unique(targetdata$Year)) #using each year as the rowname
...
for(i in 1:n.years){
  ...
  tgtRB <- sum(targetdata[targetdata$Pos=="RB" & targetdata$Year==sorted.year[i],]$Tgt,na.rm=T)
  
  # so this is saying: subset our original dataset -- targetdata[] -- by taking 
  ...
  tgtOther <- sum(targetdata[targetdata$Pos!="RB" & targetdata$Year==sorted.year[i],]$Tgt,na.rm=T)
  
  # this does the same, except the only difference is that we're taking all the
  # rows of "targetdata" where "Pos" is anything other than "RB"
